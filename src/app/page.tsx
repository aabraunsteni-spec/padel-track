...
export default function Home() {
  ...
  />
                      ...
                      <a href={`/jugador/${j.nombre.toLowerCase()}`} className="group-hover:text-[#bef264] uppercase transition-colors tracking-tighter text-sm md:text-lg">
                        {j.nombre}
                      </a>
                    </td>
      ...
      <Section title="Últimos Resultados">
        <div className="w-full max-w-2xl space-y-4 px-4">
          {partidos.map((p) => {
            const eq1Gano = p.sets_1[1] > p.sets_2[1];
            return (
